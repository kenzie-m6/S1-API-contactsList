...
describe("/users", () => {
  ...
  test("POST /users -> Should NOT be able to create an account without fullName field", async () =>{
    ...
    const response = await request(app).post("/users").send(userWithoutFullName);
  ...
  test("GET /users/user -> Should be able to list your own profile", async ()=> {
    const response = await request(app).get("/users/user").set("Authorization", await token())
  ...
  test("PATCH /users -> Should be able to update profile", async ()=> {
    ...
    const response = await request(app).patch("/users").send(newUserData).set("Authorization", await updateUserToken())
